...
def read_csv(
    file_name: str, engine_type: str = "pandas"
) -> Union[pd.DataFrame, polars.dataframe.frame.DataFrame]:
    """
    ...
    engines = {"polars": polars, "pandas": pd}

    return engines.get(engine_type.lower()).read_csv(file_name)


def generate_summary(
    dataframe: Union[pd.DataFrame, polars.dataframe.frame.DataFrame],
    column: Union[list, str] = None,
):
